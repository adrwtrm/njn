...
public final class WirelessSearchingState extends SearchingState {
    ...
    protected String getIPAddressForSearch() {
        String cvtIPAddr = NetUtils.cvtIPAddr(get_contextData().getLinkageData().wirelessIP);
        Intrinsics.checkNotNullExpressionValue(cvtIPAddr, "cvtIPAddr(_contextData.linkageData.wirelessIP)");
        return cvtIPAddr;
    }
    ...
    protected void onFound() {
        Lg.i("[QR] ⇨ 無線接続中状態へ");
        get_contextData().getContextListener().changeState(new WirelessConnectingState(get_contextData()));
    }
    ...
    protected boolean equalsIpAddress(D_PjInfo foundPjInfo) {
        ...
        int[] iArr = get_contextData().getLinkageData().wirelessIP;
        Intrinsics.checkNotNullExpressionValue(iArr, "_contextData.linkageData.wirelessIP");
