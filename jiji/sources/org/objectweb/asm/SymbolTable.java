...
public final class SymbolTable {
    ...
    private void copyBootstrapMethods(ClassReader classReader, char[] cArr) {
        ...
        while (true) {
            if (readUnsignedShort <= 0) {
                break;
            } else if (BootstrapMethodsAttribute.tag.equals(classReader.readUTF8(firstAttributeOffset, cArr))) {
    ...
    public void putBootstrapMethods(ByteVector byteVector) {
        if (this.bootstrapMethods != null) {
            byteVector.putShort(addConstantUtf8(BootstrapMethodsAttribute.tag)).putInt(this.bootstrapMethods.length + 2).putShort(this.bootstrapMethodCount).putByteArray(this.bootstrapMethods.data, 0, this.bootstrapMethods.length);
        }
    }
